...
SELECT 
  ...
  sc.id
FROM (VALUES
  ('โอวัลติน', 'Ovaltine', 40),
  ('โอวัลตินภูเขาไฟ', 'Ovaltine Volcano', 50),
  ('ไวท์มอลต์', 'White Malt', 45),
  ('โกโก้', 'Cocoa', 40),
  ('โกโก้คาราเมล', 'Caramel Cocoa', 50),
  ('นมชมพู', 'Pink Milk', 45),
  ('นมสด', 'Fresh Milk', 45),
  ('นมสดคาราเมล', 'Caramel Fresh Milk', 45),
  ('ป๊อกป๊อกนมสด', 'Pocky Fresh Milk', 45)
) AS dishes(name, name_en, price)
CROSS JOIN menu_categories mc 
CROSS JOIN subcategories sc 
WHERE mc.name = 'เครื่องดื่มไม่มีกาแฟ' AND sc.name = 'เย็น' AND sc.category_id = mc.id;

-- Insert non-coffee blended drinks
INSERT INTO menu_items (name, name_en, price, category_id, subcategory_id) 
SELECT 
  name,
  name_en,
  ...
  sc.id
FROM (VALUES
  ('โอวัลติน', 'Ovaltine', 55),
  ('โอวัลตินภูเขาไฟ', 'Ovaltine Volcano', 55),
  ('ไวท์มอลต์', 'White Malt', 55),
  ('โกโก้', 'Cocoa', 55),
  ('โกโก้คาราเมล', 'Caramel Cocoa', 55),
  ('นมชมพู', 'Pink Milk', 55),
  ('นมสด', 'Fresh Milk', 55),
  ('นมสดคาราเมล', 'Caramel Fresh Milk', 55),
  ('โอริโอ้นมสด', 'Oreo Fresh Milk', 55),
  ('ป๊อกป๊อกนมสด', 'Pocky Fresh Milk', 55)
) AS dishes(name, name_en, price)
CROSS JOIN menu_categories mc 
CROSS JOIN subcategories sc 
WHERE mc.name = 'เครื่องดื่มไม่มีกาแฟ' AND sc.name = 'ปั่น' AND sc.category_id = mc.id;

-- Insert tea cold drinks
INSERT INTO menu_items (name, name_en, price, category_id, subcategory_id) 
SELECT 
  name,
  name_en,
...
FROM (VALUES
  ('ชาดำ', 'Black Tea', 30),
  ('ชาดำน้ำผึ้ง', 'Honey Black Tea', 40),
  ('ชามะนาว', 'Lemon Tea', 40),
  ('ชาไทย', 'Thai Tea', 40),
  ('ชาเขียว', 'Green Tea', 40),
  ('ชานมไต้หวัน', 'Taiwan Milk Tea', 40),
  ('ชาไทยทูโทน', 'Two-tone Thai Tea', 45),
  ('ชาเขียวทูโทน', 'Two-tone Green Tea', 45),
  ('ชากุหลาบน้ำผึ้ง', 'Honey Rose Tea', 45),
  ('ชากุหลาบน้ำผึ้งมะนาว', 'Honey Lime Rose Tea', 45)
) AS dishes(name, name_en, price)
CROSS JOIN menu_categories mc 
CROSS JOIN subcategories sc 
WHERE mc.name = 'ชา' AND sc.name = 'เย็น' AND sc.category_id = mc.id;

-- Insert tea blended drinks
INSERT INTO menu_items (name, name_en, price, category_id, subcategory_id) 
SELECT 
  name,
  name_en,
...
CROSS JOIN menu_categories mc 
CROSS JOIN subcategories sc 
WHERE mc.name = 'ชา' AND sc.name = 'ปั่น' AND sc.category_id = mc.id;
...
FROM (VALUES
  ...
  ('แดงมะนาวโซดา', 'Red Lime Soda', 35),
  ('เขียวมะนาวโซดา', 'Green Lime Soda', 35),
  ('มะนาวโซดา', 'Lime Soda', 40),
  ('ยูซุโซดา', 'Yuzu Soda', 50),
  ('น้ำผึ้งยูซุโซดา', 'Honey Yuzu Soda', 50),
  ('น้ำผึ้งมะนาวโซดา', 'Honey Lime Soda', 50),
  ('ลิ้นจี่โซดา', 'Lychee Soda', 40),
  ('ส้มโซดา', 'Orange Soda', 40),
  ('สตรอว์เบอร์รี่โซดา', 'Strawberry Soda', 30),
  ('บลูฮาวายโซดา', 'Blue Hawaii Soda', 30),
  ('องุ่นโซดา', 'Grape Soda', 30),
  ('แอปเปิ้ลโซดา', 'Apple Soda', 30),
